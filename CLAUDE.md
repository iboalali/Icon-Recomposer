...
| `textures.js` | our own texture materials (wood, stone, paper, fabric, ceramic, enamel, carbon, holographic, mirror metals, jelly) as layer lists, their seeded SVG masks, the finish sheen, the metal Shuffle hooks (`metalVars`), the mirror metals' studio (`metalSurface`), and neon's face |
...
- A texture can build on an ambient.css material (`ambientMaterial`): anodized aluminum on brushed, radial brushed or blasted, brushed chrome, gold and copper on brushed, jelly on glass (so, like glass, it takes no surface class). The shape then gets both that `amb-mat-*` class and `ir-textured`.
- Chrome, gold and copper paint the reflected studio (`studio`: a horizon or a softbox) as the shape's own inline `background` (`metalSurface`), which wins over the surface class, and set `--amb-albedo` to the metal's mid tone so edge highlights and layers derive from the metal, not the shape color. ambient.css's brushed grain (its `::before`/`::after`, at `z-index: -1`) still draws over that background.
- Felt's fuzz is a child of the shape reaching past its outline, masked to the band outside it with a vector ring mask (`ringMask`), so it paints over the shape's own drop shadow; something drawn behind the shape would be grayed by it. Jelly's colored shadow is a blurred, offset copy of the outline behind the shape (`underlayMarkup`). Denim stitching is an SVG seam mask in canvas units (`stitchMask`), because Chrome rounds sub-pixel CSS borders down and they vanish.
