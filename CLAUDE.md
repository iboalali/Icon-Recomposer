...
- **Export filename suffixes** — `<name>-vd.xml`, `<name>-svg.svg`, `<name>-iwb.png` (background), `<name>-iwt.png` (transparent); project = `<name>.icjson` (was `.json`; old `.json` projects still open — detection is content-based). Driven by an editable **document name** field.
- **File System Access integration (Chromium only; `ui.js`)** — the **Open** button uses `showOpenFilePicker` to keep a `FileSystemFileHandle` (`currentHandle`). **Save** then overwrites that file (`createWritable`; read-write permission requested lazily on the Save gesture); **Save As** (`showSaveFilePicker`) writes a new file and re-binds. On denied permission / write error / no handle / unsupported browser, Save falls back to the old download (`downloadProject`). A **`.icjson` `file_handlers`** entry in the manifest + a `launchQueue` consumer let an installed PWA open a double-clicked project (delivered with a handle). **"Watch external edits"** (`#btn-watch`, opt-in) polls the handle's `lastModified` (`pollWatch`, 700 ms) and reloads via `reloadFromWatch` (lighter than `loadDocument` — preserves zoom/pan); any in-app edit (`commit`/`commitGesture`) auto-stops it. All of it is feature-detected (`canPick`/`fsaSupported`); the buttons stay hidden where the API is absent.
