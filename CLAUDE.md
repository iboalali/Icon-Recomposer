...
- `variant.crossings[]` holds `{ over, under }` shape id pairs that override the paint order where two shapes overlap (woven designs, which no single stack can express). `render.js` `crossingMarkup` draws `over` again right after `under`, inside a `div.ir-clip` with `under`'s geometry and `overflow: hidden`, so only `over`'s body and shadow inside `under`'s outline land on top. The copy is placed in the clip box's rotated frame; its light rotation still uses the shape's own angle. A patch belongs to `under`'s adaptive layer. `M.isOver` gives the effective order (override, else paint order), `hitTest` uses it, and deleting a shape drops its crossings. Overlap detection (`shapesOverlap`) samples a grid, so ellipses count only where they really overlap.
