...
- Wood is our own material, not ambientcss's: the shape gets `ir-wood` plus its surface class, and a `div.ir-grain` inside it multiplies a darker shade of `--amb-albedo` through an SVG mask, so the grain follows the surface shading: a seamless noise tile for planks (`WOOD_TILE`) or one ring image for end grain (`WOOD_RINGS`). The mask sits on a rotated square (`woodMarkup`, `woodAngle`) that `.ir-grain` clips to the shape. Varnish is a `div.ir-varnish.amb-mat-shiny` above the grain, so ambient.css's sheen is not darkened by it.
