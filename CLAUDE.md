...
| `textures.js` | our own texture materials (wood, stone, fibers, carbon, holographic, enamel) as layer tables, their SVG masks, the finish sheen, and neon's face |
...
- Texture materials (`textures.js`) are ours, not ambientcss's: the shape gets `ir-textured` plus its surface class, and `div.ir-tex` inside it holds the material's layers. Each layer is a square covering the shape at any angle, turned by `texAngle`, that paints a color derived from `--amb-albedo` through an alpha mask (an SVG noise tile, or one large image for wood rings and marble veins) and blends it over the surface, usually multiply or screen, so the texture keeps the surface's shading. Masks that never change are `.ir-m-*` classes; masks whose threshold follows a slider (terrazzo chips, pits, speckles, pores) are generated per value, cached, and go inline, so they are quoted with `'`. Noise tiles repeat seamlessly only when each frequency times the tile size is a whole number. The matte, satin or glossy finish is a `div.ir-sheen.amb-mat-shiny` above the texture, so ambient.css's sheen is not darkened by it. Light-dependent layers (carbon tows, cardboard ridges, holographic bands, enamel's highlight, concrete pit rims) compute from the shape's local light in JS.
- Materials share generic style fields (`texAngle`, `texScale`, `texAmount`, `finish`, `texTone`, `accent`), each read in the material's own way and labeled per material in the inspector. `M.materialDefaults()` resets them when a material is picked. `woodAngle`, `woodScale` and `woodFinish` from older files are read into them in `normalizeStyle`.
