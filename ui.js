...
function buildInspector(root) {
  ...
  const lookSec = section('Look', [
    ...
    amount('Fade', 'denim'),
    showWhen(isMat('denim', 'leather'), checkbox('Stitching', styleCtl('stitching'))),
    showWhen(() => selectedShapes().some((s) => ['denim', 'leather'].includes(s.style.material) && s.style.stitching), color('Thread color', styleCtl('accent'))),
    ...
    finish('Finish', 'wood', 'marble', 'granite', 'terrazzo', 'hammered'),
    finish('Clear coat', 'carbon'),
    finish('Sheen', 'leather'),
    ...
    amount('Pores', 'cork'),
    amount('Dent depth', 'hammered'),
    amount('Relief depth', 'leather', 'stucco'),
    amount('Depth', 'carved'),
    ...
    advanced: [
      ...
      scale('Dent size', 'hammered'),
      scale('Grain size', 'leather'),
      scale('Relief size', 'stucco'),
      scale('Chisel size', 'carved'),
