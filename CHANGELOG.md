...
- **Wood material.** Wood grain in a darker shade of the shape's own color, so the color picks the species, from pale pine to walnut. **Figure** is a plank or end grain (growth rings around an off-center pith), and **Finish** is matte, satin or glossy, with a gloss that sits over the grain. The grain follows the surface's shading and turns with the shape. **Grain direction** sets its angle, or for rings the side the pith sits on; under Advanced, **Texture strength** sets how strong it is and **Grain scale** how wide the growth lines are.
- **More materials**, in a material picker sorted into Basic, Metal, Stone, Natural, Fabric and Special:
  - **Enamel**: hard, even color with a crisp highlight, set in an optional metal rim (gold, silver, black nickel or your own color) that catches the light.
  - **Ceramic**: a soft, broad glaze highlight (glossy, satin or matte), glaze that pools darker toward the edges and thins at the rim, optional stoneware speckles, **Crackle** (a network of fine crazing lines) and **Reactive glaze** (mottling and glaze runs).
  - **Marble** (dark or light veins that wander and swell), **granite**, **terrazzo** (chip color and amount), **slate**, **concrete** (with pits) and **cork** (with pores).
  - **Paper**: plain, kraft (with dark flecks) or laid (fine laid and chain lines), fine fibers, optional **Crumpled** creases and one or two **Folds**, all lit by the scene light. **Cardboard** is a kraft liner, optionally folded, with **Exposed flutes** as an option.
  - **Carbon fiber**: a twill whose tows light up with the light direction, with an optional clear coat.
  - **Holographic**: bands, swirl, glitter or prism in rainbow, pastel, oil slick, gold foil, silver foil or two colors of your own, on the shape's color or a silver base. The colors follow the light or hold a fixed angle, with settings for how strongly they shift, how sharp the bands are, and fine diffraction lines.
  - **Neon**: a self-lit face with a glow in its own color.
  - **Chrome**, **gold** and **copper**: mirror metals that reflect a studio, bent by the surface and turned toward the light. The **Studio** is a horizon (a bright sky, a dark horizon line and a floor) or a softbox (broad light panels in a dark room, as in product shots). Each metal is polished, satin or brushed. Under Advanced, **Environment contrast** sets how strong the reflection is, **Horizon** or **Light position** moves it, and **Horizon sharpness** or **Light sharpness** goes from a crisp edge to a soft gradient. Chrome can take a **Tint** of the shape's color; gold is yellow, rose, white or the shape's color; copper can grow a green **Patina**.
  - **Anodized aluminum**: brushed, radial brushed or bead-blasted metal in the shape's color, with a bright, saturated sheen.
  - **Jelly**: a translucent, tinted gel with a glowing inner body, a soft rim and highlight on the lit side, and a shadow in its own color. **Translucency** sets how much shows through, **Inner glow** how bright the body is, and **Frost** how blurred the backdrop is.
  - **Denim**: a twill of dyed warp and pale weft threads, with patches of **Fade**, worn edges and optional **Stitching** in a thread color of your choice.
  - **Canvas**: an even over-under weave of rounded threads with holes between them, lit by the scene light, with a **Thread contrast** setting.
  - **Felt**: a dense mat of short fibers with a soft, slightly fuzzy outline (**Fuzz**).

  Every texture uses the shape's color, follows the surface's shading, turns with the shape, and carries fine detail that stays sharp at export size and when zoomed in. Each material shows only the settings it uses and starts from its own defaults when picked.
- Stone and wood materials and carbon fiber take a matte, satin or glossy finish.
- **Shuffle pattern** gives a patterned material a new random layout: new grain, veins, speckles, chips, fibers or creases, new streaks for brushed metal (also brushed chrome, gold, copper and anodized aluminum), and a new spin center and hotspot for radial brushed metal. A duplicated shape gets its own layout, so copies don't look identical. The layout is saved with the project and is the same in every variant.
- Blasted metal has a finer grain.
