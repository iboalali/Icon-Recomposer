...
- **More materials**, in a material picker sorted into Basic, Metal, Stone, Natural and Special: enamel, ceramic (glossy, satin or matte glaze, optional stoneware speckles), marble (dark or light veins), granite, terrazzo (chip color and amount), slate, concrete (with pits), cork (with pores), paper, cardboard (corrugation that catches the light), carbon fiber (a twill whose tows light up with the light direction, optional clear coat), holographic (a rainbow sheen that shifts with the light) and neon (a self-lit face with a glow in its own color). Every texture uses the shape's color, follows the surface's shading and turns with the shape. Each material shows only the settings it uses and starts from its own defaults when picked.
- Stone and wood materials, carbon fiber and ceramic take a matte, satin or glossy finish.
